...
extension GameScene {
    ...
    func setupLifePos(_ node: SKSpriteNode, i: CGFloat, j: CGFloat) {
        ...
        // Восстановим исходные размеры установки сердечек
        node.setScale(0.5)  // Убедимся, что масштаб остаётся как был ранее
        node.position = CGPoint(x: -width/2.0 + node.frame.width * i + j - 15.0,
                                y: height/2.0 - node.frame.height - 60.0)  // Опустим сердечки немного ниже, без увеличения размеров
    ...
    func setupScore() {
        ...
        coinIcon.position = CGPoint(x: -playableRect.width/2.0 + coinIcon.frame.width,
                                        y: playableRect.height/2.0 - lifeNodes[0].frame.height - coinIcon.frame.height - 70.0)  // Adjusted position

        scoreLbl.position = CGPoint(x: -playableRect.width/2.0 + coinIcon.frame.width * 2.0 - 10.0,
                                        y: coinIcon.position.y + coinIcon.frame.height/2.0 - 78.0)  // Adjusted position
    ...
    func setupPause() {
        ...
        pauseNode.position = CGPoint(x: playableRect.width/2.0 - pauseNode.frame.width/2.0 - 30.0,
                                     y: playableRect.height/2.0 - pauseNode.frame.height - 70.0)  // Increased the negative offset
